...
function App() {
  ...
  const speakText = (text: string) => {
    ...
    speakNative(cleanText, isCyrillic ? 'uk' : 'en');
  };
  ...
  const speakNative = (text: string, lang: string) => {
    ...
    utterance.voice = isUA 
      ? (ukVoices.find(v => v.name.includes("Google")) || ukVoices[0])
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        role: 'model',
        text: welcomeTab === 'ua' ? "Помилка з'єднання." : "Connection error.",
        timestamp: new Date()
      }]);
    } finally {
      setIsLoading(false);
    ...
    <div className="h-[100dvh] flex flex-col bg-teal-50/50 overflow-hidden">
      <header className="flex-shrink-0 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex items-center justify-between z-20">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleResetChat} className="p-2 rounded-full bg-slate-50 text-slate-400 hover:text-teal-600 transition-all"><RotateCcw className="w-5 h-5" /></button>
            ...
            <div className="max-w-2xl mx-auto">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-6 py-8">
                  ...
                  <p className="text-xl font-bold text-slate-800">{welcomeTab === 'ua' ? "Вітаю! Я TriPsy." : "Hello! I am TriPsy."}</p>
                </div>
              ) : (
                messages.map(msg => <ChatMessage key={msg.id} message={msg} language={welcomeTab} />)
              )}
              {isLoading && <div className="flex justify-start mb-6 pl-4 animate-pulse">...</div>}
            </div>
          </div>
